...
instance subSpec_add_left : spec₁ ⊂ₒ (spec₁ + spec₂) where
  ...
  liftM_map | q => by intro f; cases q; rfl
...
instance subSpec_add_assoc : spec₁ + (spec₂ + spec₃) ⊂ₒ spec₁ + spec₂ + spec₃ where
  monadLift
    ...
    | ⟨.inr (.inr t), f⟩ => ⟨.inr t, f⟩
  liftM_map
    | ⟨.inl t, f⟩ => by intro g; rfl
    | ⟨.inr (.inl t), f⟩ => by intro g; rfl
    | ⟨.inr (.inr t), f⟩ => by intro g; rfl
...
instance subSpec_sigma {σ ι} (specs : σ → OracleSpec ι) (j : σ) :
  ...
  liftM_map | .mk t f => by intro g; rfl
