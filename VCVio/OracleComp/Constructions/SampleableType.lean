...
@[reducible] def SampleableType.ofEquiv {α β : Type} [SampleableType α]
...
instance (α : Type) (n : ℕ) [SampleableType α] : SampleableType (Vector α n) where
  ...
  | succ m ih =>
      let pushFn : Vector α m → α → Vector α (m + 1) := fun xs x => xs.push x
      have hpush : Function.Injective2 pushFn := by
        intro xs ys x y hxy; simp [Vector.push_eq_push.mp hxy]
      rw [← Vector.push_pop_back x, ← Vector.push_pop_back y]
      change
        Pr[= pushFn x.pop x.back |
          pushFn <$>
            (Nat.recAux (pure #v[]) (fun m ih ↦ Vector.push <$> ih <*> $ᵗ α) m) <*> $ᵗ α] =
        Pr[= pushFn y.pop y.back |
          pushFn <$>
            (Nat.recAux (pure #v[]) (fun m ih ↦ Vector.push <$> ih <*> $ᵗ α) m) <*> $ᵗ α]
      rw [probOutput_seq_map_eq_mul_of_injective2 _ _ _ hpush x.pop x.back,
